...
export async function POST(req: NextRequest) {
  try {
    ...
    const safeEntreprise = escapeHtml(String(entreprise || 'Non renseigné'))
    const safeSujet = escapeHtml(String(sujet || 'Non renseigné'))
    const safeMessage = escapeHtml(String(message)).replace(/\n/g, '<br />')

    if (!apiKey) {
      console.log('📧 [Contact] Nouveau message:', { prenom: safePrenom, nom: safeNom, email: safeEmail, entreprise: safeEntreprise, sujet: safeSujet })
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [toEmail],
        reply_to: email,
        subject: `[ShiftC] Nouveau contact — ${safeSujet} — ${safePrenom} ${safeNom}`,
        html: `
